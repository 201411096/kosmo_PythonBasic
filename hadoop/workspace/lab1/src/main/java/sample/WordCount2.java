...
public class WordCount2 {
	...
	public static class MyReducer // inner class : 원래 분리해줘야됨 
	 extends Reducer<Text, LongWritable, Text, LongWritable>{
		...
		private LongWritable result = new LongWritable();	// 반환하는데 사용되는 value값
		
		@Override
		public void reduce(Text key, Iterable<LongWritable> values, Context context) throws IOException, InterruptedException{
			//Iterable enumeration 비슷?.. -> 특정 값을 뽑아내는?..
			int sum = 0;
			for(LongWritable v : values) {
				sum += v.get();		// hadoop -> java (자료형)
			}
			result.set(sum);		// java -> hadoop (자료형)
	...
	public static void main(String[] args) throws Exception{
		...
		Job job = Job.getInstance(conf, "WordCount2");	//job에 이름을 바꾼다?..
		...
		// 입력포맷과 출력포맷을 지정
		job.setInputFormatClass(KeyValueTextInputFormat.class);	//입력포맷이 텍스트가 아닌 key-value로 들어오기때문에 수정
